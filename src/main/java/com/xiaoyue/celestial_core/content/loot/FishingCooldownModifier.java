...
public class FishingCooldownModifier extends LootModifier {
    public static final MapCodec<FishingCooldownModifier> CODEC = RecordCodecBuilder.mapCodec(inst -> codecStart(inst)
            .and(BuiltInRegistries.ITEM.byNameCodec().fieldOf("condition").forGetter((m) -> m.condition))
            .and(BuiltInRegistries.ITEM.byNameCodec().fieldOf("result").forGetter((m) -> m.result))
